...
</div>  <hr style="opacity:0.1;margin:10px 0;">  <div style="line-height:1.6;">  
...
</div>  <div style="margin-top:12px;display:flex;gap:6px;flex-wrap:wrap;">  <button onclick="addMoney('${phone}')" style="background:#00d2ff;">➕</button>
...
text:"➖ 1000 FC retiré par admin",
date:Date.now()
});

await logAction("remove_money",{phone,amount:1000});

alert("✅ -1000 FC retiré");
...
// message utilisateur
await push(ref(db,"messages/"+phone),{
text:"💰 +1000 FC ajouté par admin",
date:Date.now()
});

await logAction("add_money",{phone,amount:1000});

alert("✅ +1000 FC ajouté");
...
// 🧹 supprimer données liées (optionnel mais PRO)
await safeDelete("messages/"+phone);
await safeDelete("orders/pending/"+phone);
await safeDelete("orders/validated/"+phone);

// 📊 log
await logAction("user_deleted",{phone});
...
</div>  <hr>  💰 Montant : <b>${r.amount} FC</b><br>
💳 Solde actuel : <b>${balance} FC</b><br>
📅 Date : <b>${date}</b><br>
📌 Statut : <b style="color:orange;">${status}</b><br>
🆔 ID : <small>${id}</small>

${r.proof ?   <br><br>📸 Preuve :   <br><img src="${r.proof}" style="width:100%;border-radius:10px;">   : ""}

<div style="margin-top:12px;display:flex;gap:5px;">  <button class="ok" onclick="valRecharge('${id}','${r.user}',${r.amount})">  
✅ Valider  
</button>  <button class="no" onclick="refRecharge('${id}','${r.user}',${r.amount})">  
❌ Refuser  
</button>  </div>  </div>  `;
}
});
...
details +=   📸 ${key} :<br>   <img src="${value}" style="width:100%;border-radius:10px;margin-top:5px;">   <br>   <a href="${value}" download="preuve.png">   <button style="margin-top:5px;">⬇️ Télécharger</button>   </a><br><br>  ;
}else{
...
<div class="card">  <!-- EXPEDITEUR -->  <div style="display:flex;align-items:center;gap:10px;">  
${fromAvatar}  
<div>  
<b>${fromName}</b><br>  
📱 ${t.from}  
</div>  
</div>  <div style="text-align:center;margin:10px 0;">⬇️</div>  <!-- RECEVEUR -->  <div style="display:flex;align-items:center;gap:10px;">  
${toAvatar}  
<div>  
<b>${toName}</b><br>  
📱 ${t.to}  
</div>  
</div>  <hr>  💰 Montant : <b>${amount} FC</b><br>
📅 Date : <b>${date}</b><br>
...
${details || "Aucun détail"}  
</div>  <div style="margin-top:10px;display:flex;gap:5px;">  
<button class="ok" onclick="valTrans('${id}','${t.from}','${t.to}',${amount})">  
✅ Valider  
</button>  <button class="no" onclick="refTrans('${id}')">  
❌ Refuser  
</button>  
</div>  </div>  
`;  }
...
</div>  <hr>  💰 Demande de monétisation<br>
...
✅ Approuver  
</button>  <button class="no" onclick="refMonet('${id}','${phone}')">  
❌ Refuser  
</button>  
</div>  </div>  
`;  }
